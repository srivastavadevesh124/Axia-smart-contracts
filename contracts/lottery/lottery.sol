...
contract LotteryNFT is ERC721, Ownable {
    ...
    mapping (uint256 => bool) public claimInfo;
     ...
     constructor() public ERC721("AXC LOTTERY", "AXC LOTTERY") {}
...
contract Lottery is LotteryOwnable, Initializable {
    ...
    function buy(uint256 _price, uint8[4] memory _numbers) external inDrawingPhase {
        ...
        axc.safeTransferFrom(address(msg.sender), address(this), _price);
    ...
    function  multiBuy(uint256 _price, uint8[4][] memory _numbers) external inDrawingPhase {
        ...
        axc.safeTransferFrom(address(msg.sender), address(this), totalPrice);
    ...
    function claimReward(uint256 _tokenId) external {
        ...
        if(reward>0) {
            axc.safeTransfer(address(msg.sender), reward);
        }
    ...
    function  multiClaim(uint256[] memory _tickets) external {
        ...
        if(totalReward>0) {
            axc.safeTransfer(address(msg.sender), totalReward);
        }
    ...
    function setAdmin(address _adminAddress) public onlyOwner {
        adminAddress = _adminAddress;
        emit adminChanged(_adminAddress);
    }

    // Withdraw without caring about rewards. EMERGENCY ONLY.
    function adminWithdraw(uint256 _amount) public onlyAdmin {
        axc.safeTransfer(address(msg.sender), _amount);
